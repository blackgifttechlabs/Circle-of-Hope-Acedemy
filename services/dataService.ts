...
const normalizeMainstreamGrades = (grades?: string[]): string[] => {
  const normalized = uniqueNonEmpty(grades || []);
...
export const createSubAdmin = async (name: string, pin: string): Promise<{success: boolean, message: string}> => {
  try {
    ...
    const newAdmin = {
      ...
      name: cleanName,
      pin: cleanPin,
      adminRole: 'sub_admin' as const,
    };

    const updatedAdmins = [...(settings.admins || []), newAdmin];
    const saved = await saveSystemSettings({ admins: updatedAdmins });
    if (!saved) {
    ...
    await addActivityLog({
      category: 'ADMIN',
      action: `${settings.adminName || 'Main admin'} created admin ${cleanName}`,
      actorId: 'admin',
...
export const updateAdminAccount = async (
  ...
  try {
    ...
    if (isMainAdmin) {
      const saved = await saveSystemSettings({ adminName: cleanName, adminPin: cleanPin });
      if (!saved) {
        return { success: false, message: 'Failed to save admin account.' };
      }
    } else {
  ...
  if (adminId === 'admin') {
    ...
    if (subAdmin) {
      return {
        name: subAdmin.name,
        id: subAdmin.id,
        adminRole: subAdmin.adminRole || 'sub_admin'
      };
    }
  }
  return { 
  ...
  try {
    const payload = stripUndefinedFields({
      ...
      grades: settings.grades
        ? normalizeMainstreamGrades(settings.grades)
        : undefined,
    ...
    await setDoc(doc(db, SETTINGS_COLLECTION, 'general'), payload, { merge: true });
    return true;
  } catch (error) {
