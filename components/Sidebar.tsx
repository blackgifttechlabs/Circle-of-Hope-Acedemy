...
import {
  ...
  ChevronRight, Calendar, BarChart3, BookOpen,
  HeartPulse
} from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, role, user, onLogout }) => {
  ...
  useEffect(() => {
    ...
    if (role === UserRole.ADMIN && location.pathname.startsWith('/admin/lesson-plans')) {
      localStorage.setItem(adminLessonPlansViewKey, String(Date.now()));
      setLessonPlanBadgeCount(0);
    }
  ...
  }, [role, location.pathname, adminHomeworksViewKey, adminLessonPlansViewKey, teacherHomeworksViewKey]);
  ...
  }, [role, user, adminHomeworksViewKey, adminLessonPlansViewKey, teacherHomeworksViewKey, location.pathname]);
  ...
  if (role === UserRole.ADMIN)      links = isSubAdmin ? subAdminLinks : adminLinks;
  ...
  const rolePillLabel: Record<string, string> = {
    [UserRole.ADMIN]:       isSubAdmin ? 'Sub Administrator' : 'Administrator',
            ...
            {links.map((link) => (
              <li key={link.path} className="relative group">
                <NavLink
                  ...
                  className={({ isActive }) => {
                    const linkIsActive = isActive || link.activePrefixes?.some((path) => location.pathname.startsWith(path));
                    return [
                  ...
                  {({ isActive }) => (
                    ...
                    <>
                      ...
                      <span className={[
                        ...
                        linkIsActive ? 'text-[15px] text-white' : 'text-[13.5px] text-white/55 group-hover:text-white/90',
                      ].join(' ')}>
                        {link.label}
                      </span>
                      ...
                      {!isCollapsed && link.badge > 0 && (
                        <span className={`ml-auto shrink-0 bg-red-500 text-white text-[10px] font-semibold px-1.5 py-0.5 rounded-full min-w-[18px] text-center leading-none ${link.label === 'Homework' || link.label === 'Homeworks' || link.label === 'Lesson Plans' ? 'animate-pulse' : ''}`}>
                          {link.badge}
                        </span>
                      )}
                      ...
                      {/* badge dot – collapsed */}
                      {isCollapsed && link.badge > 0 && (
                        <span className={`absolute top-2 right-2 w-[7px] h-[7px] rounded-full bg-red-500 border-[1.5px] border-[#2b2b5e] ${link.label === 'Homework' || link.label === 'Homeworks' || link.label === 'Lesson Plans' ? 'animate-pulse' : ''}`} />
                      )}
                    </>
                      );
                    })()
