...
export const printSchoolReceipt = async (
  student: Student,
  receipt: Receipt,
  totals: { paid: number; balance: number },
  options: { mode?: 'download' | 'open' } = {}
) => {
  const doc = new jsPDF();
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ['Total fees paid to date', totals.paid.toFixed(2)],
      ['Amount left to pay', totals.balance.toFixed(2)],
    ],
    columnStyles: {
      1: { halign: 'right' },
    },
